...
CONST = {
    "model_output_dir": "/home/ec2-user/SageMaker/tmp/output",
    "train_data_dir": "/home/ec2-user/SageMaker/tmp/data",
    "resource_config": "/home/ec2-user/SageMaker/tmp/resourceconfig.json",
    "hyperparameters_config": "/home/ec2-user/SageMaker/tmp/hyperparameters.json"
}
...
#    snapshot_path = outdir+'/snapshots'
...
#    gethostname_ld_preload = CONST["gethostname_ld_preload"]
    ...
    with open(hc_path, 'r') as f:
        hyperparamters = {}
        print("Hyperparameters: \n{}".format(hyperparamters)) # Debug

        try:
            backbone = hyperparamters['backbone']
        except KeyError:
            backbone = 'resnet50'
        try:
        ...
        try:
            ...
            gpus_per_host = 1
        try:
            if_name = hyperparamters['if_name']
        except KeyError:
            if_name = 'eth0'
...
-x NCCL_MIN_NRINGS={gpus_per_host} \\
...
python ./train.py --backbone {backbone} --steps {steps} --epochs {epochs} --batch-size {batch_size} --no-evaluation --snapshot-path {outdir} {dataset} {train_data_dir}"""
...
#                print(f"mpirun exit code:{exitcode}")
        ...
        # Save last snapshot if it exists
        if os.path.isfile(os.path.join(snapshot_path, 'model.h5')):
            try:
                print("saving model ...")
